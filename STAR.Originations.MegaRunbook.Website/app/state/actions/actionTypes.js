
export const BEGIN_AJAX_CALL               = 'BEGIN_AJAX_CALL'
export const END_AJAX_CALL                 = 'END_AJAX_CALL'
export const AJAX_CALL_ERROR               = 'AJAX_CALL_ERROR'

export const GET_CONFIG_SUCCESS            = 'GET_CONFIG_SUCCESS'
                                           
export const GET_USER_SUCCESS              = 'GET_USER_SUCCESS'
export const GET_LOOKUPS_SUCCESS           = 'GET_LOOKUPS_SUCCESS'
                                           
export const IS_INITIALIZED                = 'IS_INITIALIZED'

export const GET_DATA_SUCCESS              = 'GET_DATA_SUCCESS'

export const GET_RANDOM_TABLE_DATA_SUCCESS = 'GET_RANDOM_TABLE_DATA_SUCCESS'
export const GET_RANDOM_TABLE_DATA         = 'GET_RANDOM_TABLE_DATA'

export const UPDATE_APP_DIMENSIONS         = 'UPDATE_APP_DIMENSIONS'

// -------------------------------------------------------------------------------

export const UPDATE_SELECTED_APP_GROUP     = 'UPDATE_SELECTED_APP_GROUP'
export const UPDATE_SELECTED_APP_TYPE      = 'UPDATE_SELECTED_APP_TYPE'
export const UPDATE_APP_SEARCH_TEXT        = 'UPDATE_APP_SEARCH_TEXT'
export const UPDATE_APPLICATION_LINKS      = 'UPDATE_APPLICATION_LINKS'

// -------------------------------------------------------------------------------
export const GET_COURSES_SUCCESS           = 'GET_COURSES_SUCCESS'
export const GET_COURSE_SUCCESS            = 'GET_COURSE_SUCCESS'
export const GET_AUTHORS_SUCCESS           = 'GET_AUTHORS_SUCCESS'
export const CREATE_COURSE_SUCCESS         = 'CREATE_COURSE_SUCCESS'
export const UPDATE_COURSE_SUCCESS         = 'UPDATE_COURSE_SUCCESS'

export const GET_COURSE_DEMOS_SUCCESS      = 'GET_COURSE_DEMOS_SUCCESS'
export const GET_COURSE_DEMO_SUCCESS       = 'GET_COURSE_DEMO_SUCCESS'
export const UPSERT_COURSE_DEMO_SUCCESS    = 'UPSERT_COURSE_DEMO_SUCCESS'

// -------------------------------------------------------------------------------

export const GET_RELEASE_BLOCK_SUCCESS    = 'GET_RELEASE_BLOCK_SUCCESS'
export const GET_RELEASE_SUCCESS          = 'GET_RELEASE_SUCCESS'

export const GET_ACTIVITIES_SUCCESS       = 'GET_ACTIVITIES_SUCCESS'
export const GET_RELEASE_CONTACTS_SUCCESS = 'GET_RELEASE_CONTACTS_SUCCESS'

// -------------------------------------------------------------------------------

export const SIGNALR_LINE_DATA_RECEIVED   = 'SIGNALR_LINE_DATA_RECEIVED'
export const SIGNALR_PIE_DATA_RECEIVED    = 'SIGNALR_PIE_DATA_RECEIVED'

// -------------------------------------------------------------------------------

export const POST_SLACK_MESSAGE           = 'POST_SLACK_MESSAGE'
export const SEND_TWILIO_MESSAGE          = 'SEND_TWILIO_MESSAGE'

// -------------------------------------------------------------------------------

export const GET_BING_AUTHORIZATION_TOKEN_SUCCESS = 'GET_BING_AUTHORIZATION_TOKEN_SUCCESS'

// -------------------------------------------------------------------------------

export const GET_RFCS_SUCCESS               = 'GET_RFCS_SUCCESS'
export const GET_RFC_SUCCESS                = 'GET_RFC_SUCCESS'
export const GET_RUNBOOK_TEMPLATES_SUCCESS  = 'GET_RUNBOOK_TEMPLATES_SUCCESS'
export const GET_RUNBOOK_TEMPLATE_SUCCESS   = 'GET_RUNBOOK_TEMPLATE_SUCCESS'
export const GET_RUNBOOK_STEP_SUCCESS       = 'GET_RUNBOOK_STEP_SUCCESS'
export const UPDATE_RUNBOOK_STEP_SUCCESS    = 'UPDATE_RUNBOOK_STEP_SUCCESS'
export const GET_APPLICATION_GROUPS_SUCCESS = 'GET_APPLICATION_GROUPS_SUCCESS'
export const GET_APPLICATION_LINKS_SUCCESS  = 'GET_APPLICATION_LINKS_SUCCESS'
export const INSERT_RFC__SUCCESS            = 'INSERT_RFC__SUCCESS'

// -------------------------------------------------------------------------------

